...
from base import GoldenGenerator, TestConfig, ToleranceConfig, generate_input_tensor
from config import TOLERANCE_CONFIGS, STANDARD_SHAPES
from numpy_reference import gelu, gelu_tanh, quick_gelu

# PyTorch is optional - only used for cross-validation
try:
    import torch
...
class GELUGenerator(GoldenGenerator):
    ...
    def generate_numpy_reference(self, config: Dict[str, Any]) -> TestConfig:
        ...
        if input_value is not None:
            x = np.full(shape, input_value, dtype=np.float32)
        else:
            x = generate_input_tensor(shape, input_type, seed=self.seed)
    ...
    def generate_pytorch_reference(self, config: Dict[str, Any]) -> TestConfig:
        if not HAS_TORCH:
            raise NotImplementedError("PyTorch not available for cross-validation")
