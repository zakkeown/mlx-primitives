"""JAX parity tests for Learning Rate Schedulers.

This module tests parity between MLX scheduler implementations and expected
mathematical formulas. Since schedulers are pure mathematical functions,
we verify them against known formulas rather than JAX implementations.
...
class TestCosineAnnealingLRParity:
    ...
    def test_basic_cosine_annealing(self, T_max, min_lr, skip_without_jax):
    ...
    def test_cosine_with_warm_restarts(self, T_max, skip_without_jax):
        """Test CosineAnnealingLR with T_mult > 1 for warm restarts."""
        from mlx_primitives.training import CosineAnnealingLR
        ...
        # Test at beginning of first cycle
        lr_start = mlx_sched.get_lr(0)
        np.testing.assert_allclose(lr_start, base_lr, rtol=1e-6,
            err_msg="CosineAnnealingLR should start at base_lr")

        # Test at end of first cycle (should be min_lr)
        lr_end_cycle1 = mlx_sched.get_lr(T_max - 1)
        # At the end of cycle, LR should be at or near min_lr
        assert lr_end_cycle1 <= base_lr, "LR should decrease during cycle"
...
class TestOneCycleLRParity:
    ...
    def test_one_cycle_lr(self, total_steps, pct_start, skip_without_jax):
    ...
    @pytest.mark.parity_jax
    @pytest.mark.forward_parity
    def test_one_cycle_linear(self, skip_without_jax):
        ...
        for step in range(step_up, total_steps):
            lr = mlx_sched.get_lr(step)
            assert lr <= prev_lr * 1.001, f"LR not decreasing at step {step}"
            prev_lr = lr
...
class TestMultiStepLRParity:
    ...
    def test_multi_step_lr(self, milestones, gamma, skip_without_jax):
...
class TestExponentialDecayLRParity:
    ...
    def test_exponential_lr(self, gamma, skip_without_jax):
    ...
    @pytest.mark.parity_jax
    @pytest.mark.forward_parity
    def test_exponential_lr_staircase(self, skip_without_jax):
...
class TestPolynomialDecayLRParity:
    ...
    def test_polynomial_lr(self, power, total_steps, skip_without_jax):
...
class TestLinearWarmupLRParity:
    ...
    def test_linear_warmup(self, warmup_steps, skip_without_jax):
...
class TestSchedulerEdgeCases:
    ...
    def test_step_beyond_total(self, skip_without_jax):
    ...
    @pytest.mark.parity_jax
    @pytest.mark.edge_case
    def test_step_zero(self, skip_without_jax):
    ...
    @pytest.mark.parity_jax
    @pytest.mark.edge_case
    def test_very_small_lr(self, skip_without_jax):
