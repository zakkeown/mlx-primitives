...
def _get_gqa_kernel_tiled():
    ...
    if _gqa_kernel_tiled is None:
        source = """
            ...
            // Layout: [BLOCK_KV * head_dim]
            threadgroup float K_shared[32 * 128];  // Max head_dim = 128
            threadgroup float V_shared[32 * 128];
...
def fast_gqa_attention(
    ...
    Example:
        ...
        (2, 512, 64, 64)
    """
    assert q.ndim == 4, "Q must be (batch, seq_q, num_q_heads, head_dim)"
    assert k.ndim == 4, "K must be (batch, seq_kv, num_kv_heads, head_dim)"
    assert v.ndim == 4, "V must be (batch, seq_kv, num_kv_heads, head_dim)"
    ...
    assert num_q_heads == num_kv_heads * num_kv_groups, \
        f"num_q_heads ({num_q_heads}) must equal num_kv_heads ({num_kv_heads}) * num_kv_groups ({num_kv_groups})"
