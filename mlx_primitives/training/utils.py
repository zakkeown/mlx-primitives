...
- Shared utilities: copy_params, flatten_dict, unflatten_dict
"""
...
def copy_params(params: Dict[str, Any]) -> Dict[str, Any]:
    ...
    Creates a new dictionary with copied mx.array values, preserving
    the nested structure of the original dictionary.

    Args:
        params: Parameter dictionary (nested dicts/lists of mx.arrays).

    Returns:
        Deep copy of the parameter dictionary.

    Example:
        >>> weights = model.parameters()
        >>> weights_copy = copy_params(weights)
    """
...
def flatten_dict(
    ...
    Converts nested dictionaries and lists to flat key-value pairs
    where keys are joined paths like "layer.weight" or "layers.0.bias".

    Args:
        d: Nested dictionary to flatten.
        parent_key: Prefix for all keys (used in recursion).
        sep: Separator for joining key parts (default: ".").

    Returns:
        Flat dictionary mapping dotted paths to mx.array values.

    Example:
        >>> nested = {"layer": {"weight": mx.array([1, 2]), "bias": mx.array([3])}}
        >>> flat = flatten_dict(nested)
        >>> # {"layer.weight": mx.array([1, 2]), "layer.bias": mx.array([3])}
    """
        ...
        if isinstance(v, dict):
            ...
            for i, item in enumerate(v):
                if isinstance(item, mx.array):
                    ...
                    items.extend(
                        flatten_dict(item, f"{new_key}{sep}{i}", sep).items()
                    )
    return dict(items)


def unflatten_dict(
    flat: Dict[str, mx.array], sep: str = "."
) -> Dict[str, Any]:
    """Unflatten a flat dictionary to nested structure.

    Reverses the flattening done by flatten_dict.

    Args:
        flat: Flat dictionary with keys like "layer.weight", "layers.0.bias"
        sep: Separator used in flattening (default: ".").

    Returns:
        Nested dictionary matching original structure.

    Example:
        >>> flat = {"layer.weight": mx.array([1, 2]), "layer.bias": mx.array([3])}
        >>> nested = unflatten_dict(flat)
        >>> # {"layer": {"weight": mx.array([1, 2]), "bias": mx.array([3])}}
    """
    result: Dict[str, Any] = {}
        ...
        for i, part in enumerate(parts[:-1]):
            ...
            if part.isdigit():
                ...
                if not isinstance(current, list):
                    raise ValueError(f"Expected list at {sep.join(parts[:i])}")
                while len(current) <= idx:
                    current.append({} if not is_next_numeric else [])
                current = current[idx]
            else:
                # Current part is a dict key
...
class EMA:
    ...
    def __init__(
        ...
        self.shadow_params = copy_params(model.parameters())
...
class GradientAccumulator:
    ...
    Example:
        ...
        >>> for step, batch in enumerate(dataloader):
        ...     loss, grads = loss_fn(model, batch)
        ...     accumulator.accumulate(grads)  # Always accumulate first
        ...     if accumulator.should_step(step):  # Check if it's time to update
...
class Checkpointer:
    ...
    def load(self, checkpoint_path: str) -> Dict[str, Any]:
        ...
        Returns:
            Metadata dictionary with step extracted from filename.

        Raises:
            ValueError: If model is None and weights cannot be loaded.
        """
        import os
        import re

        if self.model is None:
            raise ValueError("Cannot load checkpoint: model is None")
        ...
        if os.path.isfile(checkpoint_path):
            ...
            # Load optimizer state if available (same name with _optimizer suffix)
            opt_path = checkpoint_path.replace(".safetensors", "_optimizer.npz")
            if os.path.exists(opt_path) and self.optimizer is not None:
                optimizer_state = dict(mx.load(opt_path))
...
class SWA:
    ...
    def update(self, step: int) -> None:
        ...
        if self.averaged_params is None:
            self.averaged_params = copy_params(current_params)
        else:
...
class Lookahead:
    ...
    def init_slow_weights(self, model: nn.Module) -> None:
        ...
        """
        self.slow_weights = copy_params(model.parameters())
...
class SAM:
    ...
    def first_step(self, model: nn.Module, grads: Dict[str, Any]) -> None:
        ...
        self._backup_params = copy_params(model.parameters())
